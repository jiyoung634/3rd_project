	...
	$(document).ready(function() {
		...
		$("button.btn-upload").click(function() {
			...
			var os_name = $(this).parents("tr").children().eq(0).text();
		 	$("h6#score-percentage-setting-title").html(oc_title+"("+oc_date+")의 "+ "<br>" + os_name + " 시험 정보 등록");
...
<body>
				...
				<div class="page-header float-right">
					<div class="page-title">
						<ol class="breadcrumb text-right">
							...
							<li class="active">시험 날짜 및 파일 관리</li>
						</ol>
					</div>
				</div>
			</div>
									...
									<tbody>
										...
										<c:forEach var="l" items="${exam_osList}">
											<tr>
												...
												<td><button type="button" class="btn btn-info btn-sm btn-upload" data-toggle="modal" data-target="#score-percentage-setting" value="${l.os_id}" ${(l.exam_date==null&&l.exam_file==null)?"":"disabled"}>입력</button> 
													<button type="button" class="btn btn-info btn-sm btn-delete" value="${l.os_id}" ${(l.delete_status=='Y'&&l.exam_date!=null)?"":"disabled"}>삭제</button></td>
											</tr>
										</c:forEach>
										</c:if>
										<c:if test="${exam_osList_count==0}">
											<tr><td colspan="9">시험날짜/파일 관리 가능한 과목이 없습니다</td></tr>
										</c:if>	
									</tbody>
								</table>
							</div>
					...
					<div class="modal-header">
						...
						<div class="modal-body">
							...
							<div class="form-group">
								<label for="exam_date">시험 날짜</label> <input type="date"
									class="form-control" id="exam_date" name="exam_date" required>
							</div>
							<div class="form-group">
								<label for="testfile">시험 파일</label> <input type="file"
						...
						<div class="modal-footer">
							...
							<button type="submit" class="btn btn-default test-info" form="file-upload-form">확인</button>
	...
	<script>
		$( document ).ready(function() {
			...
			var fail = "${fail}";
			if (fail == 1) {
				alert("파일 업로드에 실패했습니다.(.zip 파일만 업로드 가능합니다.)");
			}
